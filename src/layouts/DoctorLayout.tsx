...
// Icons không có sẵn trong bộ icon dùng chung — định nghĩa cục bộ.
const DashboardIcon = ({ className = 'w-5 h-5' }: LocalIconProps) => (
  <svg
    className={className}
    fill="none"
    viewBox="0 0 24 24"
...
const AppointmentIcon = ({ className = 'w-5 h-5' }: LocalIconProps) => (
  <svg
    className={className}
    fill="none"
    viewBox="0 0 24 24"
...
const HomeIcon = ({ className = 'w-5 h-5' }: LocalIconProps) => (
  <svg
    className={className}
    fill="none"
    viewBox="0 0 24 24"
...
const DoctorLayout: React.FC = () => {
  ...
  const menuItems = [
    ...
    { label: 'Tư vấn sau khám', path: '/doctor/chat', icon: <ChatBubbleIcon /> },
    {
      ...
      >
        ...
        <div className="h-16 flex items-center justify-between px-6 border-b border-border">
          <Link to="/" className="flex items-center gap-2">
            <div className="bg-primary/10 p-1.5 rounded-lg border border-primary/20 w-9 h-9 flex items-center justify-center">
        ...
        <div className="p-4 border-t border-border space-y-1.5">
          <Link
            to="/"
            className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-semibold text-muted-foreground hover:bg-accent hover:text-foreground transition-colors"
          >
            <HomeIcon />
            Về trang chủ
          </Link>
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-semibold text-red-500 hover:bg-red-50 transition-colors cursor-pointer"
          >
            <LogoutIcon />
            Đăng xuất
          </button>
